...
const CameraPage: React.FC<CameraPageProps> = ({ onClose, onCapture }) => {
    ...
    const startCamera = async () => {
      try {
        ...
        if (videoRef.current) {
          ...
          // Ensure play is called for mobile compatibility
  ...
  const takePhoto = () => {
    if (videoRef.current && canvasRef.current) {
      ...
      if (context) {
        ...
        flash.className = 'absolute inset-0 bg-white z-[80] animate-fade-in pointer-events-none';
  ...
  return (
    <div className="fixed inset-0 z-[70] bg-black flex flex-col font-sans animate-fade-in touch-none">
      {/* Top Bar */}
      <div className="absolute top-0 left-0 right-0 z-20 p-6 pt-10 flex justify-between items-center bg-gradient-to-b from-black/60 to-transparent">
      ...
      {/* Controls */}
      <div className="h-44 bg-black flex flex-col justify-center px-8 pb-10">
         <div className="flex items-center justify-between">
            {/* Gallery Stub */}
            <div className="w-12 h-12 bg-gray-800 rounded-lg border border-white/10 overflow-hidden flex items-center justify-center opacity-50">
